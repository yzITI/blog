...
选择器是CSS的重要概念，它的作用就好比是一个接线员，把样式表作用在指定的HTML元素上。本章内容是CSS的一个难点，希望大家认真思考！
...
4. 多个选择器交集：对于直接相连的选择器，需要同时满足才能被样式表选择。例如，`div.efg`会选择页面上所有具有`class="efg"`属性的`div`元素，但不会选择任何其他类型元素。
